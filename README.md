...
Position commands are input into Python, which are then fed into LabVIEW using a TCP connection. The LabVIEW controller communicates with the native mount controller to move the screws and scale the step size to reach the desired point. A homing mode was also added to set the global origin of the adjuster screws and define the full range of the screw. The absolute position is stored to an external file, so it does not need to be re-callibrated. This is important because the controller still tries to move the screw even if it cannot be turned anymore. This can cause damage to the equipment, so it is necessary to know when a screw is approaching the end of its range. The homing system is summarized in Fig. X below.  
![image](https://user-images.githubusercontent.com/67109992/221064107-aead4971-6fdb-46fe-85ea-6436184debf2.png)
...
![image](https://user-images.githubusercontent.com/67109992/221063997-f3c0454c-bd5e-4cca-8f77-7cbd893c187a.png)

The overall system setup is shown in Fig. X below. Detailed descriptions of every component in the system can be found **HERE**. The steps taken can be summarized as follows:
...
![image](https://user-images.githubusercontent.com/67109992/221064196-bcc0a231-6718-40c8-996f-fb27634dae0f.png)
